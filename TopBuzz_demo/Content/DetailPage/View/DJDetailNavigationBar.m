...
- (instancetype)initWithFrame:(CGRect)frame itemInfo:(DJCollectionItemInfo *)info {
    ...
    if (self) {
        ...
        [_backBtn addTarget:self action:@selector(backController) forControlEvents:UIControlEventTouchUpInside];
...
- (void)backController {
    if ([self.delegate respondsToSelector:@selector(popDetailViewController)]) {
        [self.delegate popDetailViewController];
    }
}
